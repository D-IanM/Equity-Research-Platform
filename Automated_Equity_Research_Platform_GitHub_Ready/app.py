...
with st.expander("What do these inputs mean?"):
    st.markdown(
        """
        **Ticker** — the stock symbol of the company you want to analyze (e.g. `AAPL` for Apple).
        This is the main company the whole report is built around.

        **Peer tickers** — a comma-separated list of similar companies used for comparison.
        These feed the Peers tab (comparable valuation) and the Risk tab
        (correlation heatmap and portfolio optimization). Leave weaker/irrelevant peers out —
        they should be real competitors in the same industry for the comparison to be meaningful.

        **DCF assumptions** (Discounted Cash Flow — a valuation method that estimates what a
        company is worth today based on the cash it's expected to generate in the future):
        - **FCF growth** — how fast you expect the company's free cash flow to grow each year
          during the explicit forecast period (first 5 years).
        - **Discount rate** — the required annual return used to convert future cash flows into
          today's dollars. Higher discount rate = lower valuation, since future money is
          worth less the more you demand for taking on risk.
        - **Terminal growth** — the slower, steady growth rate assumed forever *after* the
          forecast period ends. This must be lower than the discount rate, or the model breaks.

        **Benchmark** — the index or ticker (default `SPY`, the S&P 500 ETF) that the company's
        performance and risk metrics (like beta and Sharpe ratio) are measured against.

        **Risk-free rate** — the annual return of a "safe" investment (like a US Treasury bond),
        used as the baseline for risk-adjusted return calculations (e.g. Sharpe ratio).

        **Include SEC** — pulls recent SEC filings (10-K, 10-Q, 8-K, etc.) and financial facts
        straight from SEC EDGAR for the ticker. Requires a `SEC_USER_AGENT` secret (your name +
        email) to be set — SEC requires all automated requests to self-identify.

        **Include FRED** — pulls macroeconomic context from the Federal Reserve's FRED database:
        the Federal Funds Rate, 10-Year Treasury Yield, CPI (inflation), and Unemployment Rate.
        Requires a `FRED_API_KEY` secret (free from the FRED website).
        """
    )
...
with tabs[6]:
    ...
    if not include_fred:
        st.info("Check \"Include FRED\" in the sidebar and click Run Research to load macro data.")
    elif result.macro_data.empty:
        st.warning(
            "No FRED data was returned. Check that FRED_API_KEY is set correctly in Secrets, "
            "then reboot the app (Streamlit Cloud caches the pipeline object, so a stale "
            "cache from before the secret was set can hide new data)."
        )
    else:
        st.plotly_chart(macro_series_chart(result.macro_data), use_container_width=True)
        st.dataframe(result.macro_data, use_container_width=True)
...
st.markdown("---")
st.caption(
    "Built by Ian Dsouza & [Abhay Gadam](https://www.linkedin.com/in/abhay-gadam-42b0a3294)"
)
